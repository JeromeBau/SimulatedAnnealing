...
class VariableAnnealing(object):
    ...
    def generate_data(self, features, info, noise):
    ...
    def energy(self, measure='aic', on='subset'):
        ...
        """
        logging.debug('ENERGY - Measuring Energy on ' + str(on))
        if on == 'subset':
            logging.debug('ENERGY - Translating ' + str(self.subset))
            if sum(self.subset) == 0:
                return 99999999
            model = sm.OLS(self.y, self.X[self.translate_to_variables(self.subset)])
            logging.debug('ENERGY - Translating to ' + str(self.translate_to_variables(self.subset)))
        elif on == 'neighbor':
            logging.debug('ENERGY - Translating ' + str(self.neighbor))
            if sum(self.neighbor) == 0:
                return 99999999
            model = sm.OLS(self.y, self.X[self.translate_to_variables(self.neighbor)])
            logging.debug('ENERGY - Translating to ' + str(self.translate_to_variables(self.neighbor)))
        else:
    ...
    def anneal(self):
        ...
        while T > self.temperature_terminal:
            for n in range(len(self.subset)):
                logging.debug('ANNEAL - Current subset: ' + str(self.subset))
                self.generate_gibbs_neighbor(n)
                logging.debug('ANNEAL - Generated neighbor: ' + str(self.neighbor))
                energy_old = self.energy(on='subset')
                logging.debug('### Energy old: ' + str(energy_old))
                energy_new = self.energy(on='neighbor')
                logging.debug('### Energy new: ' + str(energy_new))
                if self.acceptance_probability(energy_old, energy_new, T) > random.random():
                    logging.debug('###### New energy won')
    ...
    def _test_0_info(self, n=3):
        """ Test the case where no column is inforamtive"""
        logging.info('Testing: Case where no column is inforamtive.')
        results = []
        for i in range(n):
            param1 = random.randint(3, 7)
            param2 = 0
            param3 = random.randint(1, 5)
            res = self._test(features=param1, info=0, noise=param3)
            logging.info('Results for features = ' +str(param1) + ', info = 0, and noise = ' + str(param3))
            logging.info('Actual optimum: ' +str(res[0]) + ', best guess: ' + str(res[1]))
            results.append([param1, param2, param3, res, res[0] == res[1]])
...
#         param1 = f
#         param2 = int((t+1)*f/5)
#         all[param1][param2] = v._test_full_search(features=param1, info=param2)
#         logging.info(str(param1) + ' -- ' + str(param2) + ' -- ' + str(all[param1][param2]))
#     p = pd.DataFrame(all)
#     p.to_csv('/home/jjb/Desktop/Search/FULL_SEARCH_'+str(f)+'.csv')



v = SimulatedAnnealingGibbs()
v.generate_data(30,15,1, 2017)
results = []
param1 = 30
param2 = 15
param3 = 1
...
t1 = time.time()
logging.info('')
print(param1, param2, best_guess, t1-t0)